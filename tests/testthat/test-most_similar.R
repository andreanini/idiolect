test_that("most_similar works", {
  ...
  d <- vectorize(enron,
    ...
    remove_numbers = T, lowercase = T, n = 5, cross_boundaries = FALSE, weighting = "rel", trim = T,
    threshold = 1500
  )
  ...
  d <- vectorize(enron,
    n = 8, cross_boundaries = FALSE, trim = F, tokens = "character", remove_punct = F,
    remove_symbols = T, remove_numbers = T, lowercase = T, weighting = "rel"
  )
