test_that("RBI works", {
  ...
  # dfm version
  d <- vectorize(c(q.data, k.data, cand.imps),
    tokens = "character", remove_punct = F, remove_symbols = T,
    remove_numbers = T, lowercase = F, n = 5, cross_boundaries = FALSE, weighting = "rel", trim = T,
    threshold = 1500
  )

  results.dfm <- impostors(
    q.data = quanteda::dfm_subset(d, quanteda::docnames(d)
...
test_that("KGI works", {
  ...
  d <- vectorize(c(q.data, k.data, cand.imps),
    tokens = "character", remove_punct = F, remove_symbols = T,
    remove_numbers = T, lowercase = F, n = 4, cross_boundaries = FALSE, weighting = "tf-idf", trim = F
  )
...
test_that("IM works", {
  ...
  # dfm version
  d <- vectorize(c(q.data, k.data, cand.imps),
    tokens = "character", remove_punct = F,
    remove_symbols = T, remove_numbers = T, lowercase = F, n = 4,
    cross_boundaries = FALSE, weighting = "tf-idf",
    trim = F
  )
