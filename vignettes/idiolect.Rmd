...
In this vignette instead demonstrates the workflow using a small data set of the Enron corpus that is included in this package (see `?enron.sample`).

```{r}
corpus <- enron.sample
```

This corpus (or the corpus inserted by the user with `create_corpus()`) is a `quanteda` corpus object that contains ten authors each with a roughly equal amount of 'known' data and with one sample that is labelled as 'unknown', as can be seen using `quanteda`'s `docvars()` function

```{r paged.print=TRUE}
docvars(corpus) |> dplyr::select(author, texttype) |> table()
```
...
Two methods for content masking are implemented in `idiolect`, the *POSnoise* algorithm developed by @halvani2021 and the *frame n-grams* approach introduced by @nini2023. Both of these options are available in the `contentmask()` function. Because this function depends on [`spacyr`](https://spacyr.quanteda.io/reference/spacyr-package.html) and this requires downloading a parsing model for a language for the automatic tagging of Parts of Speech (e.g. nouns, adjectives, adverbs), this function is not run in this vignette. Instead, the Enron sample has already been content-makes using *POSnoise*, as can be seen from the preview of the corpus

```{r}
corpus
```

As can be seen in this example, the *POSnoise* algorithm essentially replaces all words that tend to contain meaning (nouns, verbs, adjectives, adverbs) with their Part of Speech tag (N, V, J, B) while all the other words or tokens are left unchanged. In addition to this operation, *POSnoise* contains a white list of content words that mostly tend to be functional in English, such as verbs like *do, have, make* or adverbs such as *consequently*, *therefore*.
...
In this example it is simulated that the unknown text written by the author *Kimberly.watson* is the real $Q$ text and all the other known texts written by *Kimberly.watson* are therefore the set of known texts $K$. The remaining texts from the other authors are the reference samples $R$.
...
The first step of the validation is to remove the real $Q$ text from consideration. This is the actual forensic sample to analyse and it must be therefore removed when validating the analysis. The validation set is therefore made up of only the $K$ and $R$ data sets

```{r}
validation <- K + R
```

This dataset can now be re-divided into 'fake' $Q$ texts and 'fake' $K$ texts. The Enron sample already contains this division in a metadata variable (or `docvar`) called `texttype`. It is therefore possible to create two new disjoint data sets, `validation.Q` and `validation.K` which are test data sets to base the validation on.
...
This is not the only way in which a validation analysis can be conducted. For example, one could adopt a leave-one-out approach by taking each single text and treat it as a $Q$ and then run an authorship analysis method for each one of them. This simpler approach is more suitable for this example.

## Authorship analysis

The analysis that is being validated is the same analysis that will be applied to the $Q$ text. Therefore, a choice of method has to be made depending on the right choice to analyse $Q$. In this example, the scenario simulated is a *verification*: was the unknown $Q$ text written by the $K$ author, *Kymberly.watson*? For this reason, the method chosen is one of the most successful authorship verification methods available today, the *Impostors Method* [@koppel2014], and in particular one of the latest variants of it called the *Rank-Based Impostors Method* [@potha2017; @potha2020].

This analysis can be run in `idiolect` using the function `impostors()` and then selecting the default parameter for the *algorithm* argument, "RBI". The main argument of this function are the *q.data*, which is the set of $Q$ texts to test, and the *k.data*, which is the set of $K$ texts from one or more authors that are going to be tested, and finally the set of impostors data, *cand.imps*, which corresponds to the $R$ set.
...
The procedure followed by this function is to held out one text (if leave-one-out, otherwise the test dataset in its entirety) and then use the rest of the data (or the training data set) as a calibration dataset to calculate a *Log-Likelihood Ratio* ($LLR$). This analysis is done using the `calibrate_LLR()` function, which fits a logistic regression model to calibrate the score into a $LLR$ [@ishihara2021] using the `ROC` library [@vanleeuwen2015].
...
q.res <- impostors(Q, K, R, algorithm = "RBI", k = 50, features = T)
q.res$features
```
...
The search reveals that this character sequence is only used by the candidate author and nobody else in the reference corpus. However, the real underlying pattern is not the use of a comma followed by the possessive determiner *her* but the token sequence [*, here is*].
...
Although searching all the features returned is clearly a significant amount of work, by inspecting the list of features carefully and by using `concordance()` to explore the features in the data the analyst can spot patterns or mistakes in the analysis.

## Conclusions

A score of 1 is the maximum score possible but, depending on the calibration data, it can be turned into various magnitudes of the $LLR$.
...
This function not only returns the $LLR$ value but also the verbal labels and their interpretation.
...
The table above reveals that, assuming a prior probability for $H_p$ of 0.00001 (roughly, one out of the population of Manchester), then this $LLR$ would transform this probability to a posterior probability for $H_p$ of 0.0000326. In other words, it would not make much substantial difference for the trial.

However, if the prior probability of $H_p$ was 0.5, then these results would turn it to 0.97, which is a substantial change.
